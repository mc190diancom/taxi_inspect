...
public class FtpDownloader implements Runnable {
     ...
     * 构造函数
     *
     * @param host ftp host name, ip address or host name
     * @param port the defualt port is 21
     */
        ...
        try {
            ...
            if (FTPReply.isPositiveCompletion(ftpClient.getReplyCode())) {
                ...
                if (ftpClient.login(this.uname, this.password)) {
                    Log.d(TAG, "Connect " + host + " success!");
                    return true;
                }
            }
    ...
    public void run() {
        if (connect()) {
            ...
            try {
                ...
                if (files.length != 1) {
                    Log.e(TAG, info.getPair().localName + " remote file does not exist!");
                    return;
                }
                ...
                if (info.isNeedDownload(chp)) {
                    ...
                    Log.d(TAG, info.getPair().localName + " Download is done!");
                } else {
                    Log.d(TAG, info.getPair().localName + " Need not to download!");
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    protected class RetriveSingleStream implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                Log.d(TAG, __info.getPair().localName + " #Block"
