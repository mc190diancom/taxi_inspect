...
import com.miu360.taxi_check.adapter.CarRentaCarInfoAdapter;
...
public class CarRentaCarInfolActivity extends BaseActivity
		implements OnItemClickListener, OnRefreshListener2<ListView> {
	ListViewHolder holder;
	ArrayList<ZuLinInfo> arrayList = new ArrayList<ZuLinInfo>();
	CarRentaCarInfoAdapter adapter;
	HeaderHolder header;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_car_renta_car_infol);
		initView();
	}
	...
	private void initView() {
		...
		adapter = new CarRentaCarInfoAdapter(self, arrayList);
	...
	private void initData(final boolean refersh) {
		...
		final ZuLinInfo info = new ZuLinInfo();
		...
		AsyncUtil.goAsync(new Callable<Result<List<ZuLinInfo>>>() {
			...
			public Result<List<ZuLinInfo>> call() throws Exception {
				return WeiZhanData.queryZhulinCarInfo(info);
			}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		Intent intent = new Intent(self, CarRentaCarDetailInfoActivity.class);
		intent.putExtra("ZuLinInfo", (ZuLinInfo) arg0.getItemAtPosition(arg2));
