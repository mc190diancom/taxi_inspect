...
import com.miu360.taxi_check.ui.BasicResultShuiYunYehuActivity;
import com.miu360.taxi_check.ui.BasicYeHuInfoHaiYunActivity;
import com.miu360.taxi_check.ui.BasicYeHuInfoShuiYunActivity;
import com.miu360.taxi_check.util.UIUtils;

public class SightseeingBoatFragment extends BaseFragment implements OnClickListener {
	String[] items = { "水运轮船","海运轮船" };
...
*/
	List<ShuiYun> arrayList;
	List<HaiYun> seaArrayList;
	...
	private void initView(View root) {
		...
		seaArrayList = new ArrayList<>();
	}


	private final int ShuiYun = 0,HaiYun=1;
				...
				AsyncUtil.goAsync(new Callable<Result<List<ShuiYun>>>() {
					...
					public void onHandle(Result<List<ShuiYun>> result) {
						...
						if (result.ok()) {
							...
							if (1 == arrayList.size()) {
								Intent intent = new Intent(getActivity(), BasicYeHuInfoShuiYunActivity.class);
								intent.putExtra("ShuiYunInfo", arrayList.get(0));
								startActivity(intent);
							} else {
								Intent intent = new Intent(getActivity(),BasicResultShuiYunYehuActivity.class);
				...
				AsyncUtil.goAsync(new Callable<Result<List<HaiYun>>>() {
					...
					public void onHandle(Result<List<HaiYun>> result) {
						...
						if (result.ok()) {
							...
							seaArrayList.addAll(result.getData());
							if (1 == seaArrayList.size()) {
								Intent intent = new Intent(getActivity(), BasicYeHuInfoHaiYunActivity.class);
								intent.putExtra("HaiYunInfo", seaArrayList.get(0));
								startActivity(intent);
							} else {
