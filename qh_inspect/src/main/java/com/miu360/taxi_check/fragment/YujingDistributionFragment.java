...
public class YujingDistributionFragment extends BaseFragment implements OnClickListener, OnTouchListener {
	...
	private void initMapListener() {
		...
		alarmReason.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					Windows.singleChoice(getActivity(), "请选择预警原因", Items, new OnDialogItemClickListener() {
		...
		final LinearLayout ly_yujing =(LinearLayout)act.findViewById(R.id.ly_yujing);
		final HeaderHolder header = ((WarningInspectActivity)act).header;
				...
				public void onHandle(Result<List<VehiclePositionModex1>> result) {
					...
					if (result.ok()) {
						...
						for (int i = 0; i < arrayList.size(); i++) {
							...
							// ssList.add(arrayList.get(i).getGSPDATA().getVname());
						}
						// Log.e("ssList",
						// "ssList:"+result.getData().size()+"==="+ssList);
						key = 0;
						bestDistance = 0;
	...
	private void Position() {
		/*
		 * LatLng CorrectLatLng = new LatLng(39.90978, 116.433737); MapStatus
		 * mapStatus = new
		 * MapStatus.Builder().target(CorrectLatLng).zoom(16).build();
		 * mBaiduMap.setMapStatus(MapStatusUpdateFactory.newMapStatus(mapStatus)
		 * ); UIUtils.toast(act, "39.90978, 116.433737", Toast.LENGTH_SHORT);
		 * mine_position.setVisibility(View.VISIBLE);
		 */
		if (mBaiduMap != null) {
