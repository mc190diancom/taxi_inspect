...
import com.miu360.legworkwrit.mvp.model.api.Api;
...
//                    }
//                })
                .formatPrinter(new MyDefaultFormatPrinter())
                // 这里提供一个全局处理 Http 请求和响应结果的处理类,可以比客户端提前一步拿到服务器返回的结果,可以做一些操作,比如token超时,重新获取
                .globalHttpHandler(new GlobalHttpHandlerImpl(context))
                // 用来处理 rxjava 中发生的所有错误,rxjava 中发生的每个错误都会回调此接口
                // rxjava必要要使用ErrorHandleSubscriber(默认实现Subscriber的onError方法),此监听才生效
                .responseErrorListener(new ResponseErrorListenerImpl())
                .gsonConfiguration(new GlobalGsonConfiguration())
                .retrofitConfiguration(new GlobalRetrofitConfiguration())
                .okhttpConfiguration(new GlobalOkhttpConfiguration());
                //.rxCacheConfiguration(new GlobalRxCacheConfiguration());
