...
@ActivityScope
public class SimpleProcessPresenter extends BasePresenter<SimpleProcessContract.Model, SimpleProcessContract.View> {
    @Inject
    ...
    @Inject
    public SimpleProcessPresenter(SimpleProcessContract.Model model, SimpleProcessContract.View rootView) {
    ...
    public void initData(Activity activity, Case c) {
     ...
     * 初始化文书选择情况
     */
    public void initCaseList(ArrayList<BlType> types, CheckBox cbLiveCheckRecord, CheckBox cbAdministrativePunishDecide) {
        if (types != null && types.size() > 0) {
            ...
            for (BlType type : types) {
                if (Config.LIVERECORD.equals(type.getCOLUMNNAME())) {
                    ...
                    cbAdministrativePunishDecide.setChecked(true);
                    chooseInstrument("行政处罚决定书", Config.ADMINISTRATIVE, Config.ID_ADMINISTRATIVE, type.getFLAG());
                }
            }
